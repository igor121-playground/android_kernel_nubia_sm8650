				    ...
				    unsigned long start, unsigned long end)
{
	...
	for (page_addr = start; page_addr < end; page_addr += PAGE_SIZE) {
		...
		ret = list_lru_add(&binder_freelist, page_to_lru(page));
		WARN_ON(!ret);
				      ...
				      unsigned long addr)
{
	...
	case -EBUSY:
		...
		npages = get_user_pages_remote(alloc->mm, addr, 1,
					       FOLL_NOFAULT, &page, NULL, NULL);
		if (npages <= 0) {
			pr_err("%d: failed to find page at offset %lx\n",
			       alloc->pid, addr - (uintptr_t)alloc->buffer);
			ret = -ESRCH;
			break;
		}
		fallthrough;
				    ...
				    unsigned long start, unsigned long end)
{
	...
	for (page_addr = start; page_addr < end; page_addr += PAGE_SIZE) {
		...
		if (page) {
			...
			on_lru = list_lru_del(&binder_freelist, page_to_lru(page));
			WARN_ON(!on_lru);
